    ...
    match $mux {
        "zellij" => {
            ...
            let fzf_opts = [
                ...
                $"--preview-window=right:($SESSION_CONFIG.preview_size):wrap"
                "--prompt='🖥️  Select Zellij Session: '"
                "--header='Enter: Switch │ Ctrl-C: Cancel │ Ctrl-N: New Session'"
                "--bind='ctrl-n:execute(echo __CREATE_NEW__)'"
                $preview_cmd
            ] | where {|x| $x != ""} | str join " "
            ...
            let result = if $use_floating {
                if (multiplexer is-active) {
                    ...
                    # Fixed zellij floating syntax with separate width/height flags
                    if $use_large {
                        ^zellij run --floating --width "120" --height "40" --name "session-switcher" --close-on-exit -- bash $script_file
                    } else {
                        ^zellij run --floating --width "100" --height "30" --name "session-switcher" --close-on-exit -- bash $script_file
                    }
                    rm -f $script_file $temp_file
                    return
                } else {
